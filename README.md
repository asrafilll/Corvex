...
- **Editable Task details** with descriptions, status, priority, and due dates.
- **Global command palette** for workspace search and Project-aware quick Task capture.
- **Project Activity ledger** showing append-only UI and MCP changes, including the MCP Token responsible for agent work without recording sensitive values.

## How it's scoped (single-user by design)

- **Single user.** No orgs, roles, registration, Profile, sharing, or admin panel. The platform app is the Corvex UI and opens with one app password.
...
- `apps/api`: Hono API on Node.js, Prisma 7 + Postgres, app-password session auth.
...
# Generate the app-password hash and two required secrets, then set them in .env:
pnpm app-password:hash           # -> APP_PASSWORD_HASH (prompts without echo)
...
In development only, leaving `APP_PASSWORD_HASH` empty uses the password `corvex`.
Production refuses this default.
...
| `APP_PASSWORD_HASH` | Scrypt app-password hash generated by `pnpm app-password:hash`. Production rejects the development default. |
| `APP_SESSION_SECRET` | Signing key for the HTTP-only browser-session cookie. Minimum 32 characters in production. |
...
```sh
pnpm test         # Vitest suites across API, Platform, and Worker
pnpm typecheck
...
```sh
cp .env.example .env
pnpm app-password:hash    # set APP_PASSWORD_HASH
openssl rand -base64 32   # set APP_SESSION_SECRET
...
For production, set the public addresses before building. Caddy serves the human API
under `/api` on the same origin and keeps the separate API host for MCP clients:
...
`POST /auth/unlock` verifies the configured scrypt hash and rate-limits failures. A
successful unlock sets a signed, HTTP-only, `SameSite=Strict` cookie with no persistent
expiry, so refreshes and new tabs stay unlocked while closing the browser or pressing
Lock asks for the app password again. The app password is never stored in the browser.
MCP ignores this cookie and continues to require a project-scoped `Bearer cvx_…` token.
